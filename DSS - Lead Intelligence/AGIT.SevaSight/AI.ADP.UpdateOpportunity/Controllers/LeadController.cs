...
namespace AGIT.HR.Web.Controllers
{
    public class LeadController : Controller
    {
       ...
       public ActionResult Index()
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult sendLead()
        {
            var response = new HttpResponseMessage();
            List<LeadResponse> listLead = new List<LeadResponse>();
            Uri APIM_Uri = new Uri(ConfigurationManager.AppSettings["APILeadUri"]);

            using (IKernel kernel = new StandardKernel())
            {
                kernel.Bind<ISalesOrderDataAccess>().To<SalesOrderDataAccess>();
                listLead = kernel.Get<SalesOrderBusiness>().FindLead();
                if(listLead != null)
                {
                    for(var i =0; i < listLead.Count; i++)
                    {
                        LeadRequest sendLead = new LeadRequest();
                        sendLead.ID = listLead[i].ID;
                        sendLead.Nama = listLead[i].Nama;
                        sendLead.KTP = listLead[i].NoKTP;
                        sendLead.NoTelp = listLead[i].NoTelp;
                        sendLead.Email = listLead[i].Email;
                        sendLead.Jenis9_Now = listLead[i].Jenis6;
                        sendLead.Series_Now = listLead[i].TipeMotor;
                        sendLead.Jenis9_Next = listLead[i].Jenis6Selanjutnya;
                        sendLead.Series_Next = "";
                        sendLead.TanggalBeliSelanjutnya = listLead[i].TanggalBeliSelanjutnya.ToString();
                        sendLead.SalesPerson = listLead[i].SalesPerson;

                        using (WebApiClient client = new WebApiClient())
                        {
                           client.BaseAddress = APIM_Uri;
                            client.DefaultRequestHeaders.Add("Ocp-Apim-Subscription-Key", "d6a52b698f8d41cf99563dba3f270afd");
                            response = client.PostAsJsonAsync("CreateLead", sendLead).Result;
                            if (response.IsSuccessStatusCode)
                            {
                                //Update StatusLead
                                SalesOrder lead = new SalesOrder();
                                lead.ID = listLead[i].ID;
                                lead.Nama = listLead[i].Nama;
                                lead.NoKTP = listLead[i].NoKTP;
                                lead.NoTelp = listLead[i].NoTelp;
                                lead.Email = listLead[i].Email;
                                lead.TanggalCetak = listLead[i].TanggalCetak;
                                lead.TipeMotor = listLead[i].TipeMotor;
                                lead.Jenis6Selanjutnya = listLead[i].Jenis6Selanjutnya;
                                lead.TanggalBeliSelanjutnya = listLead[i].TanggalBeliSelanjutnya;
                                lead.SalesPerson = listLead[i].SalesPerson;                               
                                kernel.Get<SalesOrderBusiness>().UpdateStatusLead(lead);
                            }
                            else
                            {
